...
import { Camera, RefreshCw, Check, AlertCircle, Loader2, Maximize2 } from "lucide-react";

interface MeterCameraProps {
  onCapture: (imageFile: File, previewUrl: string, metrics?: { captureMs: number | null }) => void;
  onClear?: () => void;
...
async function captureOriginalFrame(video: HTMLVideoElement, stream: MediaStream): Promise<{ file: File; previewUrl: string }> {
  ...
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob(
      (value) => (value ? resolve(value) : reject(new Error("failed to create capture image"))),
      "image/png",
    );
  });
      ...
      advanced.push({
        zoom: Math.min(
          ...
          Math.max(
            ...
            capabilities.zoom.min + (capabilities.zoom.max - capabilities.zoom.min) * 0.25,
          ),
        ),
      });
    }
...
export const MeterCamera: React.FC<MeterCameraProps> = ({
  ...
  const stopCamera = useCallback(() => {
    ...
    if (stream) {
      stream.getTracks().forEach((track) => {
        try {
          ...
          console.debug("[Mizan] Camera track cleanup skipped", error);
        }
      });
    ...
    if (video) {
      try {
        ...
        console.debug("[Mizan] Video pause cleanup skipped", error);
      }
  ...
  const capturePhoto = useCallback(async () => {
    ...
    try {
      ...
      stopCamera();
      onCapture(file, newPreview, { captureMs });
    } catch (err) {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full gap-4 p-4 border rounded-xl bg-card shadow-sm dir-rtl">
      {error && (
        <div className="w-full rounded-lg border border-red-300 bg-red-50 text-red-700 p-3 text-sm text-right dir-rtl flex items-start gap-2"><AlertCircle className="w-4 h-4 mt-0.5 shrink-0" /><span>{error}</span></div>
      )}
      ...
      {isCameraActive && (
        <div className="relative w-full max-w-2xl overflow-hidden rounded-lg bg-black aspect-video flex items-center justify-center">
          <video
            ...
            autoPlay
            muted
            aria-label="معاينة كاميرا عداد المياه"
          />
          {isStreamReady && (
      ...
      {previewUrl && (
        <div className="flex flex-col items-center gap-3 w-full max-w-2xl">
          ...
          <div className="rounded-md border bg-muted/30 px-3 py-2 text-xs text-center">
            تم التقاط صورة أصلية عالية الجودة. الصورة الأصلية محفوظة كما التقطت؛ أي نسخة معالجة لاحقاً هي نسخة عمل مشتقة فقط.
          </div>
