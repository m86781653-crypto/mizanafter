...
const allPages = ['dashboard','projects','infrastructure','customers','readings','billing','maintenance','reports','copilot','settings'] as const;
...
const roleAccess: Record<UserRole, PageId[]> = {
  ...
  operations_officer: ['dashboard','infrastructure','maintenance','reports','copilot','settings'],
  ...
  data_exception_officer: ['dashboard','readings','reports','copilot','settings'],
  viewer: ['dashboard','reports','copilot','settings'],
};

function AuthedApp() {
  const { profile, loading } = useAuth();
  const [page, setPage] = useState<PageId>('dashboard');
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-neutral-50"><div className="text-center"><div className="w-12 h-12 border-4 border-primary-200 border-t-primary-600 rounded-full animate-spin mx-auto mb-3" /><p className="text-sm text-neutral-500">جاري التحميل...</p></div></div>;
