...
begin
  ...
  if p_fault_id is not null and not exists (
    select 1 from public.faults f where f.id=p_fault_id and f.project_id=p_project_id
  ) then raise exception 'FAULT_NOT_FOUND'; end if;
  if p_asset_id is not null and not exists (
    select 1 from public.assets a where a.id=p_asset_id and a.project_id=p_project_id
  ) then raise exception 'ASSET_NOT_FOUND'; end if;
  if p_well_id is not null and not exists (
    select 1 from public.wells w where w.id=p_well_id and w.project_id=p_project_id
  ) then raise exception 'WELL_NOT_FOUND'; end if;
  if p_pump_id is not null and not exists (
    select 1 from public.pumps p where p.id=p_pump_id and p.project_id=p_project_id
  ) then raise exception 'PUMP_NOT_FOUND'; end if;
  ...
  perform private.mizan_write_audit(
    ...
    jsonb_build_object('work_order_number',v_wo.work_order_number,'fault_id',p_fault_id,
      'assigned_to',v_wo.assigned_to,'priority',v_wo.priority),'success', 'success'
  );
  return v_wo;
end;
$$;
...
begin
  ...
  if not private.mizan_can_write_project(v_project_id,'maintenance_work_orders','update') then
    raise exception 'WORK_ORDER_UPDATE_FORBIDDEN';
  end if;

  update public.maintenance_work_orders
     set status=p_status,
         completed_date=case when p_status='completed' then coalesce(completed_date,now()) else completed_date end
   where id=p_work_order_id
   returning * into v_wo;

  perform private.mizan_write_audit(
    v_project_id,'WORK_ORDER_STATUS_CHANGED','maintenance_work_order',v_wo.id,null,
    jsonb_build_object('status',p_status),'success','success'
  );
  return v_wo;
end;
$$;
