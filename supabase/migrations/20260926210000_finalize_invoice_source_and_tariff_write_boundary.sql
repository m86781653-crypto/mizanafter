-- Final billing contract reconciliation.
-- Keep the approved MRX reading identity explicit in the invoice RPC contract,
-- and prevent client roles from mutating tariff definitions directly.
...
begin
  ...
  if exists (
    ...
    where i.source_reading_id = v_reading.id
  ) then
    select * into v_invoice
    from public.invoices
    where source_reading_id = v_reading.id
    limit 1;
    return v_invoice;
  end if;
  ...
  if v_consumption <= 0 then raise exception 'NO_NEW_CONSUMPTION'; end if;
  ...
  insert into public.invoices (
    ...
    amount_paid, balance, status, source_reading_id
  ) values (
...
revoke insert, update, delete on public.tariffs from anon, authenticated;
revoke insert, update, delete on public.tariff_tiers from anon, authenticated;
